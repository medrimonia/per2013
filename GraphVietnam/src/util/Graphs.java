...
//	 */
	...
	/*
	public static <V, E extends Graph.Edge<V>> FlowResults<E> fordFulkerson_old(
			Graph<V, E> g, V source, V target, Map<E, Integer> capacities) {
		Graph<V, E> residual = new MultiGraph<V, E>();
		for (V v : g.vertices())
			residual.addVertex(v);
		for (E e : g.edges()) {
			Edge<V> e1 = new DirectedEdge<V>(e.source(), e.target());
			residual.addEdge((E) e1);
			capacities.put((E) e1, capacities.get(e));
		}

		boolean optimize = false;
		while (!optimize) {
			List<V> path = new ArrayList<V>();
			findAugmentingPath(residual, source, target, path);
			if (!path.isEmpty()) {
				int min = Integer.MAX_VALUE;
				for (int i = 1; i < path.size(); i++) {
					Edge<V> e = edge(residual, path.get(i - 1), path.get(i));
					Integer c = capacities.get(e);
					if (c < min)
						min = c;
				}
				for (int i = 1; i < path.size(); i++) {
					E e = edge(residual, path.get(i - 1), path.get(i));
					capacities.put(e, capacities.get(e) - min);
					if (capacities.get(e) <= 0)
						residual.removeEdge(e);
					if (edge(residual, path.get(i), path.get(i - 1)) == null) {
						Edge<V> e1 = new DirectedEdge<V>(path.get(i),
								path.get(i - 1));
						residual.addEdge((E) e1);
						capacities.put((E) e1, min);
					} else {
						E e1 = edge(residual, path.get(i), path.get(i - 1));
						capacities.put(e1, capacities.get(e1) + min);
					}
				}
			} else {
				optimize = true;
		...
		minimumCut(residual, source, vertices);
		FlowResults<E> result = new FlowResults<E>();
		result.maximumFlow = new HashMap<E, Integer>();
		for (E e : g.edges()) {
			result.maximumFlow.put(e, 0);
		}
		result.minimumCut = new HashSet<E>();
		for (E e : g.edges()) {
			// update minimum cut
			if (vertices.contains(e.source()) && !vertices.contains(e.target())
					|| !vertices.contains(e.source())
					&& vertices.contains(e.target()))
				result.minimumCut.add(e);
			// update maximum flow
			if (edge(residual, e.target(), e.source()) == null) {
				result.maximumFlow.put(e, 0);
				continue;
			}
			int c = capacities.get(edge(residual, e.target(), e.source()));
			result.maximumFlow.put(e,
					c <= capacities.get(e) ? c : capacities.get(e));
			// update value
			if (e.source().equals(source)) {
				result.value += result.maximumFlow.get(e);
			}
	...
	private static <V, E extends Graph.Edge<V>> E edge(Graph<V, E> g, V s, V t) {
		for (E e : g.edges()) {
			if (e.source().equals(s) && e.target().equals(t))
				return e;
		}
		return null;
	}

	private static <V, E extends Graph.Edge<V>> void findAugmentingPath(
			Graph<V, E> g, V s, V t, List<V> path) {
		path.add(s);
		if (s.equals(t)) {
			return;
		} else {
			for (Iterator<E> it = g.outgoingEdges(s).iterator(); it.hasNext();) {
				E e = it.next();
				if (!path.contains(e.target()))
					findAugmentingPath(g, e.target(), t, path);
				if (path.contains(t))
					return;
			}
		}
	...
	private static <V, E extends Graph.Edge<V>> void minimumCut(Graph<V, E> g,
			V v, List<V> set) {
		set.add(v);
		for (Iterator<E> it = g.outgoingEdges(v).iterator(); it.hasNext();) {
			E e = it.next();
			if (!set.contains(e.target()))
				minimumCut(g, e.target(), set);
		}
	}	 
	
	public static <V, E extends Graph.Edge<V>> PartialGraph<V, E> kruskal_old(Graph<V, E> g, final Map<E, Integer> weight){
		@SuppressWarnings("unchecked")
		PartialGraph<V, E> tree = g.partialGraph((Set<E>) Collections.EMPTY_SET);
		Queue<E> q = new PriorityQueue<E>(g.order(), new Comparator<E>() {
			@Override
	...
	private static <V, E extends Graph.Edge<V>> boolean isAcyclic(Graph<V, E> tree){
		Set<V> marked = new HashSet<V>();
		for (V v : tree.vertices()){
			if (!marked.contains(v)){
				return dfsAcyclic(tree, v, v, marked);
			}
		}
		return false;
	}
	
	private static <V, E extends Graph.Edge<V>> boolean dfsAcyclic(Graph<V, E> tree, V v, V u, Set<V> marked){
		marked.add(v);
		for (V w : tree.neighbors(v)){
			if (!marked.contains(w)){
				boolean f = dfsAcyclic(tree, w, v, marked);
				if (f) return true;
			}else if (!w.equals(u)){
				return true;
			}
