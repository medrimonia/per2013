...
\paragraph{}
Thereafter if it's not stated otherwise, a graph design an undirected 
simple graphs.
...
complete graph on $n$ vertices is a $K_n$.
...
length, and the path of length $k$ is denoted by $P^k$. Note that $k$ is
...
\paragraph{Cut-set :}
The cut-set of a cut $C=(S,T)$ is the set of edge defined as $\{uv \in E | u \in S ~and~ v \in T\}$.
...
%TODO Explain or change the notation G-C + put ref to disconnected
\paragraph{Articulation set :}
A vertex cut $C$ of a graph $G=(V,E)$ is a subset of $V$ such that $G-C$ becomes
disconnected.
...
A graph $G$ is connected if for every pair $\{u,v\}$ of vertices if there is a 
...
\paragraph{Flow :}
Let G we distinguish two vertices: a source $s$ and a sink $t$.
