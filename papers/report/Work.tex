...
\begin{algorithm}[!h]
    ...
    \For{i = 1 to N-1}{
        \For{j = 1 to k}{
            $edges = edges \cup (components[i][j], componets[i+1][j])$
        }
    }
...
\paragraph{}
The edges between each component are not randomly selected. Let say that the
vertices of each component are numbered from 1 to the size of the component.
The edges between the first vertex of the first and second component is selected.
The same process is repeated for the second vertices to the $k$-th vertices and
then  for the other components. At the end a $k$-connected graph is generated.
